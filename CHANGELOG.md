...
- Report an error from the SPIR-V backend when an instruction is too long to encode, instead of emitting an instruction header with a truncated word count. Previously, an entry point name of roughly 256 KiB produced a malformed module that could make Vulkan drivers read out of bounds. By @ErichDonGubler in [#10419](https://github.com/gfx-rs/wgpu/pull/10419).
