...
- Work around a use-after-free crash in Imagination (PowerVR) drivers: when `vkAllocateDescriptorSets` fails with host or device OOM, the descriptor pool is left holding a dangling pointer that a later `vkDestroyDescriptorPool` dereferences. Such a pool is now leaked rather than destroyed, and the device is lost so that at most one pool leaks. By @ErichDonGubler in [#99999](https://github.com/gfx-rs/wgpu/pull/99999).
